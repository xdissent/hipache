...
Cache.prototype.getSecureContextFromHost = function (host, cb) {
    this.readFromCache(host, function (err, rows) {
        if (err) return cb('cache read failed: ' + err.toString());
        var context;
        try {
            ...
            if (!this.config.server.https.key || !this.config.server.https.cert) {
                return cb('invalid ssl credentials: ' + err.toString());
            }
        }
        cb(null, context);
    }.bind(this));
...
Cache.prototype.readFromCache = function (hostKey, cb) {
    ...
    multi.get('cert:' + hostKey);
    multi.get('key:' + hostKey);
...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    this.readFromCache(host, function (err, rows) {
        if (err) return callback('cache read failed: ' + err.toString(), 503);
        var backends = rows[0];
