...
Worker.prototype.runServer = function (config) {
    ...
    var sniHandler = function (host, callback) {
        if (typeof callback === 'undefined') {
            log('SNICallback must be asynchronous (node >= 0.11.5)');
            return;
        }
        this.cache.getSecureContextFromHost(host, function (err, context) {
            if (err) {
                log('sniHandler: ' + err.toString());
                return callback(err);
            }
    ...
    if (config.https) {
        ...
        options.SNICallback = sniHandler;
